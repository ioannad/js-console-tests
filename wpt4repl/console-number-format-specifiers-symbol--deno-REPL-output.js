...
console.log('   --- Test using `for (let method ...)` --- ');

for (let method of methods) { // deno throws if the loop variable is not a `let` or a `const`.
  console.log("Testing %s", method);
...
console.log('   --- Test using `for (const method ...)` --- ');

for (const method of methods) { // deno throws if the loop variable is not a `let` or a `const`.
  console.log("Testing %s", method);
  console[method]("%i", Symbol.for("description"));
  if (method == "group" || method == "groupCollapsed") console.groupEnd();
  console[method]("%d", Symbol.for("description"));
